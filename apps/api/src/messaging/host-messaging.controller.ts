...
export class HostMessagingController {
  ...
  create(@CurrentUser() user: RequestUser, @Body() dto: CreateConversationDto) {
    return this.messagingService.startConversation(user.sub, UserRole.HOST, dto);
  }
  ...
  send(
    ...
    return this.messagingService.sendMessage(id, user.sub, UserRole.HOST, dto);
  }
