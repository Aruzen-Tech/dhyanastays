...
export default function AdminConversationPage() {
  ...
  const fetchConversation = useCallback(() => {
    ...
    adminMessagingApi
      ...
      .then((c) => {
        setConversation(c);
        adminMessagingApi.markRead(id).catch(() => {});
      })
  ...
  if (error || !conversation) {
    return (
      <div className="container-page py-16 text-center">
        ...
        <Link href="/admin/messages" className="btn-primary mt-4 inline-block">
          Back to messages
        </Link>
      </div>
    );
  ...
  return (
    <div className="container-page py-6 max-w-2xl mx-auto flex flex-col" style={{ height: 'calc(100vh - 80px)' }}>
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
        <Link href="/admin/messages" className="btn-ghost p-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-gray-900 text-sm truncate">
            ...
            <span className="ml-1.5 text-xs font-normal text-gray-400">
              {otherUser.role === 'HOST' ? 'Host' : otherUser.role === 'GUEST' ? 'Guest' : 'Admin'}
            </span>
          </p>
