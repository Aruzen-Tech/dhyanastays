...
import type { Booking, Hold, Listing, PriceQuote } from '../../../lib/types';

// Deterministic gradient palette — no external requests
...
type Step = 'details' | 'quote' | 'booking' | 'payment' | 'confirmed';
...
export default function ListingDetailPage() {
  ...
  const handleGetQuote = async () => {
    ...
    try {
      // guests defaults to 1 in the api wrapper; backend requires it
      const q = await pricingApi.quote({ listingId: id, checkIn, checkOut, guests: 1 });
      setQuote(q);
      setStep('quote');
    } catch (e: unknown) {
  ...
  const handleCreateBooking = async () => {
    ...
    setActionLoading(true);
    try {
      // Backend DTO field is `plan`, not `paymentPlan`
  ...
  const handleInitPayment = async () => {
    ...
    try {
      ...
      // In stub mode (local dev), razorpayOrderId starts with "stub_".
      // Auto-confirm the payment so the booking transitions to CONFIRMED_*.
      if (result.razorpayOrderId?.startsWith('stub_')) {
        await paymentsApi.stubConfirm(result.paymentId);
      }
      // In production: open Razorpay checkout with result.razorpayOrderId + result.keyId
    ...
    <div className="container-page py-10">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Hero — inline SVG placeholder (no external requests) */}
          ...
          <div className="card p-6 sticky top-24">
            ...
            {step === 'details' && (
              <div className="space-y-4">
                ...
                <button
                  ...
                  disabled={!checkIn || !checkOut || actionLoading}
              ...
              <div className="space-y-4">
                <div className="bg-brand-50 rounded-xl p-4 space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">{quote.nights} night{quote.nights !== 1 ? 's' : ''}</span>
            ...
            {step === 'booking' && hold && (
              <div className="space-y-4">
                <div className="alert-info text-xs">
                  ⏱ Dates held for 15 minutes. Complete booking now.
                </div>
            ...
            {step === 'payment' && booking && (
              <div className="space-y-4">
                ...
                <div className="alert-info text-xs">
                  🔒 Razorpay-secured payment. In stub mode, payment is simulated.
                </div>
            ...
            {step === 'confirmed' && (
              <div className="text-center space-y-4">
                ...
                <h3 className="font-bold text-gray-900">Booking initiated!</h3>
                <p className="text-sm text-gray-500">
                  Your booking is being processed. Check your dashboard for status updates.
                </p>
                <button
