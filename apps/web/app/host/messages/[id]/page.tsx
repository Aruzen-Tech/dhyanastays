...
export default function HostConversationPage() {
  ...
  const fetchConversation = useCallback(() => {
    ...
    hostMessagingApi
      ...
      .then((c) => {
        setConversation(c);
        hostMessagingApi.markRead(id).catch(() => {});
      })
  ...
  const handleSend = async (body: string) => {
    setSending(true);
    try {
      const msg = await hostMessagingApi.sendMessage(id, body);
  ...
  if (error || !conversation) {
    return (
      <div className="container-page py-16 text-center">
        ...
        <Link href="/host/messages" className="btn-primary mt-4 inline-block">
          Back to messages
        </Link>
      </div>
    );
  ...
  return (
    <div className="container-page py-6 max-w-2xl mx-auto flex flex-col" style={{ height: 'calc(100vh - 80px)' }}>
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
        <Link href="/host/messages" className="btn-ghost p-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-gray-900 text-sm truncate">
            ...
            <span className="ml-1.5 text-xs font-normal text-gray-400">
              {otherUser.role === 'GUEST' ? 'Guest' : otherUser.role === 'ADMIN' ? 'Admin' : 'Host'}
            </span>
          </p>
